...
public class UsuarioDAO {
    ...
    public String actualizarContrasenia(int idusuario,String contrasenia){
        ...
        try (Connection conn = Database.getConnection();
            ...
            stmt.setInt(2, idusuario);  // ID del usuario a actualizar

            // Ejecutar la actualización
            int rowsUpdated = stmt.executeUpdate();
            if (rowsUpdated > 0) {
                mensaje = "Su contraseña se actualizo exitosamente. Ya puede iniciar sesion con su nueva contraseña";
            } else {
                mensaje = "No se pudo actualizar la contraseña. Ocurrio un error inesperado";
            }
