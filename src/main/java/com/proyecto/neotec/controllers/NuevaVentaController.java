...
public class NuevaVentaController {
    ...
    @FXML
    private TableColumn<Productos, String> columna1; //codigo
    @FXML
    private TableColumn<Productos, String> columna2; //producto-nombre
    @FXML
    private TableColumn<Productos, Float> columna3; //precio unitario
    @FXML
    private TableColumn<Productos, Integer> columna4; //cantidad
    @FXML
    private TableColumn<Productos, Float> columna5; //total linea
    @FXML
    ...
    public void cargarLinea() {
        ...
        if (productoLinea.getNombreProducto() == null) {
    ...
    public void eliminarLinea() {
        Productos productoSeleccionado = tablaVenta.getSelectionModel().getSelectedItem();

        if (productoSeleccionado != null) {
            listaProductos.remove(productoSeleccionado);
        } else {
            MostrarAlerta.mostrarAlerta("Eliminar linea222", "Debe seleccionar una línea de la tabla para eliminar.", Alert.AlertType.WARNING);
        }
    }
    ...
    public void cerrarTicket() {
        ...
        if (result.isPresent() && result.get() == buttonSi) {
            //TODO: falta afectar a la caja

            //verificar que hay una caja establecida
            Caja cajaestablecida = CajaEstablecida.getCajaSeleccionada();
            if (cajaestablecida == null){
                MostrarAlerta.mostrarAlerta("Venta", "Para realizar una venta en efectivo debera establecer una caja en Caja->Establecer Caja", Alert.AlertType.WARNING);
            }

            //verificar apertura
            CajaDAO cajadao = new CajaDAO();
            if (cajadao.verificarApertura(cajaestablecida.getIdcaja())){
                String respuesta = cajadao.registrarMovimientoDeCaja(cajaestablecida.getIdcaja(), 0,Float.parseFloat(txfTotal.getText()),SesionUsuario.getUsuarioLogueado().getIdusuarios(),"Venta en Efectivo");
        ...
        for (Productos p1 : listaProductos) {
